...
class Type extends CI_Model{
    ...
    public function get_categories_and_count_products($id){
        $query = "SELECT product_types.id, product_types.type_name AS 'type_name', product_types.image_dir, COUNT(products.id) AS 'product_count'
            FROM products
            RIGHT JOIN product_types
            ON products.product_type_id = product_types.id
            GROUP BY product_types.id";
        $value = "";
        if($id != "all"){
            $query = $query . " HAVING product_types.id = ?";
